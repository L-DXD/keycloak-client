...
class JWTUtilTest {
   ...
   void tokenVerify() {
      ...
      StepVerifier.create(getAccessToken(EXPIRED_TOKEN))
          .assertNext(token -> {
             ...
             assertThat(token.getPreferredUsername()).isEqualTo("testuser");
   ...
   @Test
   @DisplayName("case11. Invalid Audience test")
   void invalidAudience() {
      ...
      StepVerifier.create(
          ...
          .expectErrorMatches(
              throwable -> throwable instanceof VerificationException && throwable.getMessage()
                  .equals("Expected audience not available in the token"))
          .verify();
   }
